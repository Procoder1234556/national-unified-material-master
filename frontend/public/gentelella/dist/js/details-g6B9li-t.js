import { r as a } from './main-v4-4eeF3Vvb.js';
...
var n = {
  ...
  i = { green: 'status-green', yellow: 'status-yellow', red: 'status-red' };
function l(a) {
...
var t = [
    { title: 'Wireframes review', done: !0 },
    { title: 'Stakeholder kickoff', done: !0 },
    { title: 'Design system tokens', done: !0 },
    { title: 'Component library', done: !1 },
    { title: 'QA + accessibility audit', done: !1 }
  ],
  r = [
...
function o({ title: o, client: d, status: c, sCls: v, desc: p, pct: m, due: g, members: u }) {
  const h = u
      .map(a => {
        const e = n[a] || { name: a, role: 'Team member', color: 'primary' };
    ...
    b = r
      ...
      .join(''),
    y = 'red' === v ? 'var(--red)' : 'yellow' === v ? 'var(--yellow)' : 'var(--primary)',
  ...
  a({
    ...
    actions: [
      ...
      { label: 'Open project', variant: 'primary', action: () => e(`Opening: ${o}`) }
    ]
  });
}
function d({ name: n, ini: i, color: t, role: r, projects: o, tasks: d, msgs: c }) {
  const v = (function (a) {
      return a.toLowerCase().replace(/\s+/g, '.') + '@example.com';
    })(n),
    p =
      '+1 (555) ' +
      (300 + (i.charCodeAt(0) % 600)).toString().padStart(3, '0') +
      '-' +
      (1e3 + (i.charCodeAt(i.length - 1) % 9e3)).toString(),
  ...
  a({
    ...
    actions: [
      { label: 'Close', variant: 'outline' },
      { label: 'Message', variant: 'outline', action: () => e(`Compose to: ${n}`) },
      {
        label: 'View profile',
        variant: 'primary',
        action: () => {
...
export { d as openContactModal, o as openProjectModal };
