...
import { initNummAI } from './v4/numm-ai.js';
...
// Service worker â€” only in production builds (skip on dev so HMR isn't fought
...
//
// A host that bundles this entry without shipping sw.js â€” a Laravel or Django
// app serving the design system from node_modules, say â€” opts out with
...
// Todo checkboxes â€” toggle .done on the cb + row, then refresh any
...
document.addEventListener('click', e => {
  ...
  // Format: "<remaining> of <total> remaining" â€” matches existing copy.
...
// Card option buttons â†’ popover menu.
...
// Chip dismiss (Ã— icon) and chip toggle.
document.addEventListener('click', e => {
...
// Form submit â€” let HTML5 validation run, then fake-submit on valid forms.
...
document.addEventListener('submit', e => {
  ...
  // server-rendered app â€” sign-in, registration, and every create or edit
  ...
  if (form.dataset.demoSubmit === 'false') {
    return;
  }
  ...
  import('./v4/toast.js').then(({ showToast }) =>
    showToast(`${label} âœ“`, { variant: 'success' })
  );
  if (form.dataset.resetOnSubmit !== 'false') {
    form.reset();
  }
});

// Topbar search box opens the command palette â€” wired by initCommandPalette.
