// Gentelella 2026 v4 â€” shell render (pure)
...
// exports) â€” Rolldown narrows it to the one string it sees used, so the rest
// of package.json never reaches the bundle.
import pkg from '../../package.json' with { type: 'json' };

const VERSION = pkg.version;

// NAV items are either flat â€” { key, href, text, icon, badge? } â€”
...
// Label â†’ href lookup for breadcrumb crumbs, derived from NAV so the two can't
// drift. A parent resolves to its first child â€” the page the sidebar opens when
...
//   1. explicit target in the page's data-breadcrumb â€” "Forms|form.html"
//   2. exact label match against NAV (see CRUMB_HREFS above)
//   3. no match â†’ plain text, exactly as before
...
export function renderTopbar(breadcrumb) {
  const crumbs = (breadcrumb && breadcrumb.length ? breadcrumb : ['Home'])
    .map((c, i, arr) => {
      ...
      const sep = i > 0 ? '<span class="sep" aria-hidden="true">â€º</span>' : '';
