...
function getCurrentFiles() {
  ...
  if (currentId === 'shared') {
    return FILES.filter(f => f.parent === 'marketing');
  } // demo: marketing as shared
  if (currentId === 'trash') {
    return [];
  }
...
function renderBreadcrumb() {
  ...
  document.getElementById('fm-breadcrumb').innerHTML = trail
    .map(
      (n, i) =>
    ...
    actions: [
      { label: 'Cancel', variant: 'outline' },
      {
        ...
        action: ({ body }) => {
          ...
          if (!name) {
            showToast('Name is required', { variant: 'warning' });
            return false;
          }
          f.name = name;
          renderFiles();
...
export function initFileManager() {
  ...
  document.getElementById('fm-grid').addEventListener('click', e => {
      ...
      const items =
        f.type === 'folder'
          ? [
              {
                label: 'Open',
                action: () => navigateTo(getNode(f.name.toLowerCase())?.id || currentId)
              },
          ...
          : [
              ...
              {
                label: 'Share',
                action: () => showToast(`Share link copied: ${f.name}`, { variant: 'success' })
              },
              { label: 'Rename', action: () => renameFile(id) },
              {
    ...
    if (item.dataset.type === 'folder') {
      ...
      const node =
        TREE.find(n => n.parent === f.parent && n.name === f.name) ||
      ...
      actions: [
        { label: 'Cancel', variant: 'outline' },
        {
          ...
          action: ({ body }) => {
            const name = body.querySelector('input').value.trim();
            if (!name) {
              showToast('Folder name is required', { variant: 'warning' });
              return false;
            }
            FILES.push({
              id: nextId++,
