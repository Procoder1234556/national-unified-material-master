...
function setStoredOpenIndex(idx) {
  try {
    if (idx === null) {
      ...
      sessionStorage.setItem(SUBMENU_STATE_KEY, String(idx));
    }
  } catch (_e) {
    /* private mode */
  }
...
function bindRailFlyouts() {
  ...
  document.querySelectorAll('.sidebar .nav-toggle').forEach(btn => {
    btn.addEventListener(
      'click',
      e => {
        if (!document.body.classList.contains('sidebar-rail')) {
          return;
        }
        if (!isDesktop()) {
          return;
      ...
      true
    ); // capture phase — runs before bindNavSubmenus
  });
}
...
function openShortcutsModal() {
  const row = (k, label) =>
    `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--border-color-light);font-size:13px"><span style="color:var(--text)">${label}</span><span>${k
      .split('+')
      .map(
        key =>
          `<kbd style="font-family:var(--font);font-size:11px;background:var(--bg-surface-secondary);border:1px solid var(--border-color);border-radius:3px;padding:2px 6px;margin-left:3px">${key}</kbd>`
      )
      .join('')}</span></div>`;
  showModal({
          ...
          <span class="panel-icon panel-icon-${n.kind}" aria-hidden="true">
            ${
              n.kind === 'alert'
                ...
                : n.kind === 'task'
                  ? '<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 8l3 3 7-7"/></svg>'
    ...
    body: `
      <div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:14px">
        <div style="width:36px;height:36px;border-radius:8px;background:var(--${n.kind === 'alert' ? 'red' : n.kind === 'task' ? 'green' : 'blue'}-lt);color:var(--${n.kind === 'alert' ? 'red' : n.kind === 'task' ? 'green' : 'blue'});display:flex;align-items:center;justify-content:center;flex-shrink:0">
          ${
            n.kind === 'alert'
              ? '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 1l7 13H1L8 1z"/><path d="M8 6v4"/></svg>'
              : n.kind === 'task'
                ? '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 8l3 3 7-7"/></svg>'
  ...
  showModal({
    ...
    actions: [
      ...
      },
      {
        label: 'Send reply',
        variant: 'primary',
        action: () => showToast('Reply sent', { variant: 'success' })
      }
    ]
  });
}
