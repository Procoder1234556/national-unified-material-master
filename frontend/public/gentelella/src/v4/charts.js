...
function dashboardNetwork(echarts, el, t) {
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      trigger: 'axis',
      axisPointer: { type: 'line', lineStyle: { color: t.borderLight } }
    },
    legend: { show: false },
    xAxis: {
      type: 'category',
    ...
    series: [
      {
        ...
        data: [
          ...
          },
          {
            value: 20,
            name: 'Direct',
            itemStyle: { color: t.azure, borderColor: t.bgSurface, borderWidth: 2 }
          },
          {
            value: 15,
            name: 'Referral',
            itemStyle: { color: t.yellow, borderColor: t.bgSurface, borderWidth: 2 }
          },
          {
            value: 12,
            name: 'Social',
            itemStyle: { color: t.purple, borderColor: t.bgSurface, borderWidth: 2 }
          },
          {
            value: 13,
            name: 'Email',
    ...
    series: [
      {
        ...
        barWidth: '52%',
        data: items
          .map(d => ({ value: d[1], itemStyle: { color: d[2], borderRadius: [0, 4, 4, 0] } }))
          .reverse()
      }
    ]
  });
...
function heatmap(echarts, el, t) {
  ...
  chart.setOption({
    ...
    tooltip: {
      ...baseOption(t).tooltip,
      formatter: p =>
        `${days[p.value[1]]} ${hours[p.value[0]]}:00<br><strong>${p.value[2]}</strong> events`
    },
...
function calendarHeatmap(echarts, el, t) {
  ...
  chart.setOption({
    ...
    tooltip: {
      ...baseOption(t).tooltip,
      formatter: p => `${p.value[0]}: ${p.value[1]} contributions`
    },
    visualMap: {
      min: 0,
      max: 100,
    ...
    calendar: {
      ...
      itemStyle: {
        color: t.bgSurfaceSecondary || t.borderLight,
        borderColor: t.bgSurface,
        borderWidth: 2
      },
      splitLine: { show: false },
