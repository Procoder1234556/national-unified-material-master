...
function renderCard(card) {
  const labels = card.labels
    ...
    .join('');
  const avatars = card.assignees
    .map(a => {
      const info = ASSIGNEES[a];
      return info
        ? `<span class="kanban-avatar" style="background:${COLOR_VAR[info.color]}" title="${escapeHtml(info.name)}">${a}</span>`
        : '';
    })
    .join('');
  const priorityIcon =
...
function openCardEditor(cardId) {
  ...
  actions.push({
    ...
    action: ({ body: bodyEl }) => {
      ...
      card.labels = Array.from(bodyEl.querySelectorAll('[data-group="labels"] .chip.active')).map(
        b => b.dataset.label
      );
      card.assignees = Array.from(
        bodyEl.querySelectorAll('[data-group="assignees"] .kanban-assignee-chip.active')
      ).map(b => b.dataset.assignee);
      if (isNew) {
        CARDS.push(card);
      }
      render();
