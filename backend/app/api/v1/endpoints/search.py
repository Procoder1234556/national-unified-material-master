...
from backend.app.models.models import UnifiedMasterCode
...
async def national_search(req: NationalSearchRequest, session: AsyncSession = Depends(get_db)):
    """
    ...
    for master in masters:
        if (
            req.size_inch
            and master.size_inch is not None
            and abs(float(master.size_inch) - float(req.size_inch)) > 0.01
        ):
            continue
        ...
        if (
            query_attrs.get("size_inch")
            and master.size_inch is not None
            and float(query_attrs.get("size_inch")) == float(master.size_inch)
        ):
        ...
        if has_explicit_filter or score >= 0.20 or overlap >= 1:
            ...
            from backend.app.models.models import MaterialMapping, RawMaterial
